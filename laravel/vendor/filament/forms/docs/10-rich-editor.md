...
const tiptapSharedPlugin = {
    ...
    setup(build) {
        ...
        build.onResolve(
            { filter: /^@tiptap\/(core|pm\/(state|view|model))$/ },
            (args) => ({
                path: args.path,
        ...
        build.onLoad(
            { filter: /.*/, namespace: 'tiptap-shared' },
            async (args) => {
...
esbuild.build({
    ...
    entryPoints: [
        './resources/js/filament/rich-content-plugins/my-extension.js',
    ],
    outfile:
        './resources/js/dist/filament/rich-content-plugins/my-extension.js',
});
```
