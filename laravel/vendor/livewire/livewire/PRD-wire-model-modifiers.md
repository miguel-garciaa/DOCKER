...
directive('model', ({ el, directive, component, cleanup }) => {
    ...
    let ephemeralModifiers =
        liveIndex === -1 ? modifiers : modifiers.slice(0, liveIndex);
    let networkModifiers =
        liveIndex === -1 ? [] : modifiers.slice(liveIndex + 1);
    ...
    let networkUpdate = buildNetworkUpdate(
        networkModifiers,
        component,
        expression,
        el,
...
10. `wire:model.blur.live.throttle.500ms` — ephemeral on blur, network throttled
